...
const NavItemDropdown = ({text, dropdown}) => {
    ...
    return (
        <li className="relative p-5 hover:bg-white" ref={dropDownItem}>
            <button onClick={handleClick} className="font-semibold font-secondary focus:outline-none">
                {text}
            </button>
            {showDropdown ? (
                <div className="bg-white absolute right-0 rounded-lg mt-1 z-10 py-2 w-48">
                    {dropdown.map(item => {
                        return (
                            <a key={item.key} href={item.link} className="block py-2 text-blue-800 font-secondary font-semibold px-4 hover:text-tertiary duration-150">{item.text}</a>
                        )
                    })}
                </div>
            ): null}
